...
class TestGetAvilableSlotsInteractor:
    def test_returns_avilable_slots(self):
        ...
        storage.get_list_of_datewise_slots_till_range.\
            return_value = datewise_slots_list
        presenter.get_response_of_list_of_datewise_slots_till_range. \
            return_value = expected_list

        actual_list = interactor. \
            get_slots_for_particular_days_with_avilablity_status(user_id=user_id)

        assert expected_list == actual_list
        presenter.get_list_of_datewise_slots_till_range. \
            assert_called_once_with(avilable_slots=actual_list)
        storage.get_response_of_list_of_datewise_slots_till_range. \
            assert_called_once()
